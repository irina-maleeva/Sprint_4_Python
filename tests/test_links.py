...
@allure.description('Находясь на странице заказа, нажать на лого "Самокат" и убедиться, что произошел переход на главную страницу')
def test_samokat_logo_link(driver):
    home_page = HomePage(driver)
    home_page.click_order_button_in_heading()
    order_page = OrderPage(driver)
    order_page.wait_for_page_load()
    assert driver.current_url == url + order_page.path
    order_page.click_samokat_logo()
    assert driver.current_url == url

@allure.title('Проверка перехода на главную страницу Яндекс.дзен по клику на лого "Яндекс"')
@allure.description('Находясь на странице заказа, нажать на лого "Яндекс" и убедиться, что произошел переход на главную страницу Яндекс.дзен')
def test_yandex_logo_link(driver):
    ...
    assert driver.current_url == yandex_url